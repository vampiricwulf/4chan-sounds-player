...
module.exports = {
	...
	async initialize() {
		...
		if (Player.config.showUpdatedNotification && Player.config.VERSION && Player.config.VERSION !== "3.6.0") {
			Player.alert(`4chan Sounds Player has been updated to <a href="${Player.settings.changelog}" target="_blank">version ${"3.6.0"}</a>.`);
		}
	...
	async migrate(fromVersion) {
		...
		if (!fromVersion || fromVersion === "3.6.0") {
			return {};
		}
		const changes = {};
...
module.exports = {
	...
	build(data) {
		...
		!data.ignoreVersion && (html = html.replace(/%v/g, "3.6.0"));
